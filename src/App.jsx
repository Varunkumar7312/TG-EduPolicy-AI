...
- Recommendations: Discontinue EAPCET; use strengthened IPE marks for UG admissions; regulate private coaching; merge SSC Board and BIE into Telangana School Education Board (TSEB); discontinue Class 11 public exam
...
- Features: standardized high-quality infrastructure, labs, libraries, sports; English-medium; safe transportation; no closures/forced mergers
...
- 90%+ private TEIs but public DIETs perform BETTER on TET
- Recommendations: Discontinue D.El.Ed diploma; restructure B.Ed. into stage-specific degrees; strengthen DIETs; prohibit distance/online B.Ed.; develop Teacher Competency Framework; periodic teacher certification
...
- 1,855 residential institutions, 7.45 lakh students; 40% still in rented/makeshift premises
- Recommendations: Unify all under one Telangana State Residential Education Society; limit to 1,000 students per school; mandatory mental health professionals; digital end-to-end governance

CHAPTER 8 - COLLEGIATE EDUCATION:
- 60%+ faculty positions vacant, no regular recruitment since 2013
- Recommendations: Urgent faculty recruitment; Apprenticeship Embedded Degree Programmes (AEDP); timely scholarships; unified academic calendar; NAAC accreditation push

CHAPTER 9 - TECHNICAL EDUCATION:
- 175 engineering colleges, 90%+ private; 22,000+ engineering seats VACANT in 2025-26
- Polytechnic colleges drifted from industry-ready training to JEE/ECET feeder institutions
- Recommendations: Performance-linked differentiation; outcome-based curriculum; integrate polytechnics with Telangana Skill University

CHAPTER 10 - UNIVERSITY EDUCATION:
- Major crisis: prolonged VC vacancies, dissolved Academic/Executive Councils, faculty recruitment frozen
- Recommendations: Transparent VC recruitment; fill all vacancies per UGC norms; reinstate democratic student elections; upgrade libraries, labs, hostels

CHAPTER 11 - MID-DAY MEALS & FOOD SAFETY:
- Repeated food poisoning incidents across 33 districts
- Recommendations: Revised nutritionally balanced menu (vegetables, dal, eggs, fruits daily); extend MDM to Intermediate students; additional Rs 200 crore/year investment; replace firewood with gas; SS 304 stainless steel utensils; digital payment system

CHAPTER 12 - OPEN & DISTANCE LEARNING (ODL):
- Telangana pioneered ODL with BRAOU, TOSS - now constrained by weak staffing, high dropout rates
- Recommendations: Strengthen TOSS with regular trained staff; establish CODE-T (Consortium of Distance Education - Telangana); bridge digital divide; extend to prisoners, women, marginalized communities

GENERAL RECOMMENDATIONS:
- BUDGET: Allocate at least 18% of state budget to education (currently 7.55%); 5% for higher education
- LANGUAGE: English as medium of instruction from Nursery to PG
- GOVERNANCE: IAS officers in education departments to serve minimum 5-year tenure
- RTE EXTENSION: Extend Right to Education from 6-14 years to 3-18 years
- CASTE DIGNITY: SC/ST categories not to appear on attendance registers or public documents
- BIOMETRIC ATTENDANCE: For all students, teachers, staff; link to salary
- PRIVATE SCHOOLS: No PPP/FDI in education; regulate fees through TAFRC

Answer in clear, simple language. Use bullet points when listing recommendations. Always be encouraging and positive about Telangana's future.`;
...
const chapters = [
  ...
  { id: 11, icon: "🍱", title: "Mid-Day Meals & Food Safety", color: "#854f0b", bg: "#faeeda", summary: "Rs 200 cr more/year for nutritious meals, gas stoves, steel utensils, digital payments", tag: "Nutrition" },
...
const stats = [
  ...
  { value: "Rs 200Cr", label: "Extra for meals", sub: "Per year recommended" },
];
...
const chapterDetails = {
  1: { problem: "Pre-primary education is mostly in underfunded Anganwadi centres. Private nursery schools cost Rs 20,000 to Rs 1 lakh+ per year, pushing poor families into debt from age 3.", recs: ["Add Nursery, LKG & UKG to ALL government primary schools", "Hire trained pre-school teachers (not just general teachers)", "Play-based learning — no exams, no homework at this age", "Regulate private pre-schools for safety & fee transparency", "Standard infrastructure: safe toilets, clean classrooms, play areas"] },
      ...
      <div style={{ background: chapter.bg, borderRadius: 16, padding: "20px 20px 16px", marginBottom: 16 }}>
        <div style={{ display: "flex", gap: 14, alignItems: "flex-start" }}>
          ...
          <div>
            ...
            <h2 style={{ fontSize: 20, fontWeight: 500, margin: "0 0 8px", color: "#111" }}>{chapter.title}</h2>
            <p style={{ fontSize: 14, color: "#555", margin: 0, lineHeight: 1.55 }}>{chapter.summary}</p>
          </div>
        </div>
      </div>
      <div style={{ marginBottom: 16 }}>
        <p style={{ fontSize: 12, color: "#888", textTransform: "uppercase", letterSpacing: "0.08em", margin: "0 0 10px" }}>The Problem</p>
        <p style={{ fontSize: 14, lineHeight: 1.65, color: "#222", margin: 0, background: "#f5f5f3", borderRadius: 10, padding: "14px 16px" }}>{detail?.problem}</p>
      </div>
      <div style={{ marginBottom: 20 }}>
        <p style={{ fontSize: 12, color: "#888", textTransform: "uppercase", letterSpacing: "0.08em", margin: "0 0 10px" }}>Key Recommendations</p>
        <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
          {detail?.recs.map((r, i) => (
            <div key={i} style={{ display: "flex", gap: 10, alignItems: "flex-start", background: "#f5f5f3", borderRadius: 8, padding: "10px 14px" }}>
              <span style={{ color: chapter.color, fontWeight: 500, flexShrink: 0, fontSize: 14 }}>{i + 1}.</span>
              <span style={{ fontSize: 14, color: "#222", lineHeight: 1.55 }}>{r}</span>
            </div>
          ))}
        </div>
      </div>
...
export default function App() {
  ...
  const [messages, setMessages] = useState([
    { role: "ai", text: "Namaste! 🙏 I'm your guide to the Telangana Education Policy 2026. Ask me anything — from what the policy says about English medium to how Telangana Public Schools work, or what changes are coming for universities. What would you like to know?" }
  ]);
  ...
  async function sendMessage() {
    ...
    try {
      ...
      if (data.error) {
        setMessages(prev => [...prev, { role: "ai", text: "Error: " + (data.error.message || data.error) }]);
      } else {
    ...
    <div style={{ fontFamily: "Georgia, serif", background: "#fff", minHeight: "100vh" }}>
      ...
      <div style={{ background: "#0f6e56", padding: "0 24px", position: "sticky", top: 0, zIndex: 100, display: "flex", alignItems: "center", justifyContent: "space-between", height: 52 }}>
        <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
          ...
          <span style={{ color: "#fff", fontWeight: 500, fontSize: 15 }}>Telangana Education Policy 2026</span>
        </div>
        <div style={{ display: "flex", gap: 6 }}>
          {[["home", "Overview"], ["chapters", "Chapters"], ["chat", "Ask AI"]].map(([v, label]) => (
            <button key={v} onClick={() => { setView(v); setActiveChapter(null); }} style={{ padding: "5px 14px", borderRadius: 20, border: "1px solid rgba(255,255,255,0.3)", background: view === v ? "rgba(255,255,255,0.2)" : "transparent", color: "#fff", fontSize: 13, cursor: "pointer", fontFamily: "Georgia, serif" }}>{label}</button>
          ))}
        </div>
      </div>
      ...
      {view === "home" && (
        <div>
          ...
          <div style={{ padding: "32px 24px 8px" }}>
            <p style={{ fontSize: 11, color: "#888", textTransform: "uppercase", letterSpacing: "0.1em", margin: "0 0 16px", textAlign: "center" }}>Policy at a glance</p>
            <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 10, maxWidth: 640, margin: "0 auto" }}>
              {stats.map(s => (
                <div key={s.label} style={{ background: "#f5f5f3", borderRadius: 10, padding: "14px 12px", textAlign: "center" }}>
                  <div style={{ fontSize: 22, fontWeight: 500, color: "#0f6e56", lineHeight: 1 }}>{s.value}</div>
                  <div style={{ fontSize: 11, color: "#333", fontWeight: 500, marginTop: 4 }}>{s.label}</div>
          ...
          <div style={{ padding: "24px 24px 8px", maxWidth: 640, margin: "0 auto" }}>
            <div style={{ background: "#faeeda", border: "1px solid #ef9f27", borderRadius: 12, padding: "16px 18px", marginBottom: 16 }}>
              ...
              <ul style={{ margin: 0, padding: "0 0 0 16px", fontSize: 13, color: "#633806", lineHeight: 1.7 }}>
                <li>Private school enrollment surged from 40% to 60%+ as parents lost faith in govt schools</li>
                <li>Education budget fell from 10.9% to 5.9% of state budget over a decade</li>
          ...
          <div style={{ padding: "8px 24px 32px", maxWidth: 640, margin: "0 auto" }}>
            <p style={{ fontSize: 11, color: "#888", textTransform: "uppercase", letterSpacing: "0.1em", margin: "0 0 14px" }}>Key decisions in the policy</p>
            <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
              {highlights.map((h, i) => (
                <div key={i} style={{ display: "flex", gap: 12, alignItems: "flex-start", background: "#f5f5f3", borderRadius: 8, padding: "10px 14px" }}>
                  <span style={{ fontSize: 16 }}>{h.emoji}</span>
                  <span style={{ fontSize: 13, color: "#222", lineHeight: 1.55 }}>{h.text}</span>
                </div>
              ))}
            </div>
          </div>
      ...
      {view === "chapters" && (
        <div style={{ padding: "24px 20px", maxWidth: 680, margin: "0 auto" }}>
          <h2 style={{ fontSize: 20, fontWeight: 500, margin: "0 0 6px", color: "#111" }}>All 12 Chapters</h2>
          <p style={{ fontSize: 13, color: "#888", margin: "0 0 20px" }}>Click any chapter to learn more, or ask the AI for details.</p>
          {activeChapter ? (
            <div>
              ...
              <ChapterDetail chapter={activeChapter} onAskAI={() => setView("chat")} />
            </div>
          ) : (
            <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
              {chapters.map(ch => (
                <div key={ch.id} onClick={() => setActiveChapter(ch)}
                  style={{ background: "#fff", border: "0.5px solid #e0e0e0", borderRadius: 12, padding: "14px 16px", cursor: "pointer", display: "flex", gap: 14, alignItems: "flex-start", transition: "border-color 0.15s" }}
                ...
                >
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 4, flexWrap: "wrap" }}>
                      ...
                      <span style={{ fontSize: 11, color: "#aaa" }}>Chapter {ch.id}</span>
                    </div>
                    <p style={{ fontSize: 15, fontWeight: 500, margin: "0 0 4px", color: "#111" }}>{ch.title}</p>
                    <p style={{ fontSize: 13, color: "#666", margin: 0, lineHeight: 1.5 }}>{ch.summary}</p>
                  </div>
                  <span style={{ color: "#ccc", fontSize: 16, flexShrink: 0, marginTop: 4 }}>›</span>
                </div>
              ))}
            </div>
          )}
      ...
      {view === "chat" && (
        <div style={{ display: "flex", flexDirection: "column", height: "calc(100vh - 52px)" }}>
          <div style={{ padding: "12px 20px 8px", borderBottom: "0.5px solid #e8e8e8", background: "#fff", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
            <p style={{ margin: 0, fontSize: 13, color: "#888" }}>Powered by Groq AI — trained on the full 354-page Telangana Education Policy 2026</p>
          </div>

          <div style={{ flex: 1, overflowY: "auto", padding: "16px 20px", display: "flex", flexDirection: "column", gap: 12 }} ref={chatEndRef}>
            {messages.map((m, i) => (
              <div key={i} style={{ display: "flex", justifyContent: m.role === "user" ? "flex-end" : "flex-start" }}>
                ...
                <div style={{ maxWidth: "80%", padding: "10px 14px", borderRadius: m.role === "user" ? "16px 16px 4px 16px" : "16px 16px 16px 4px", background: m.role === "user" ? "#0f6e56" : "#f1f0e8", color: m.role === "user" ? "#fff" : "#222", fontSize: 14, lineHeight: 1.6, whiteSpace: "pre-wrap" }}>{m.text}</div>
              </div>
            ))}
            {loading && (
              <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                ...
                <div style={{ background: "#f1f0e8", borderRadius: "16px 16px 16px 4px", padding: "10px 16px", fontSize: 13, color: "#888" }}>Thinking...</div>
              </div>
            )}
          </div>
          ...
          {messages.length <= 1 && (
            <div style={{ padding: "0 20px 10px" }}>
              <p style={{ fontSize: 11, color: "#aaa", margin: "0 0 8px", textTransform: "uppercase", letterSpacing: "0.06em" }}>Try asking:</p>
              <div style={{ display: "flex", flexWrap: "wrap", gap: 6 }}>
                {sampleQuestions.map(q => (
                  <button key={q} onClick={() => setInput(q)} style={{ background: "#f1f0e8", border: "0.5px solid #e0e0e0", borderRadius: 16, padding: "5px 12px", fontSize: 12, cursor: "pointer", color: "#333", fontFamily: "Georgia, serif" }}>{q}</button>
                ))}
              </div>
            </div>
          )}

          <div style={{ padding: "10px 16px 14px", borderTop: "0.5px solid #e8e8e8", background: "#fff", display: "flex", gap: 8 }}>
            <input value={input} onChange={e => setInput(e.target.value)} onKeyDown={e => e.key === "Enter" && !e.shiftKey && sendMessage()} placeholder="Ask about any part of the policy..." style={{ flex: 1, borderRadius: 22, border: "0.5px solid #ccc", padding: "10px 16px", fontSize: 14, outline: "none", fontFamily: "Georgia, serif" }} />
